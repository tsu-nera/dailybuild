...
def _unit_name(creds: Credentials, unit_id: str, cache: dict) -> str:
    ...
    try:
        body = _get(creds, f'{USER}/dataTypes/food-measurement-unit/dataPoints/{unit_id}')
...
def _nutrition_log_row(point: dict, unit_cache: dict) -> dict | None:
    ...
    log_date = _to_date(civil_start['date'])
    ...
    while True:
        ...
        for point in page:
            ...
            if 'date' not in civil_start:
                continue
            page_dates.append(_to_date(civil_start['date']))
