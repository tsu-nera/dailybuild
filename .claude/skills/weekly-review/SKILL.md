---
name: weekly-review
description: 週次レポート生成→AIレビューを実行する週次レビュースキル（GTD準拠・週末実施）
user-invocable: true
...
GTDのWeekly Reviewに合わせて**週末に実施する**（土日のどちらでもよい。ISO週は月曜始まり
なので土曜と日曜は同じ週に入り、どちらで回しても「今週」の判定は変わらない）。
今週分のデータを集約し、曜日傾向・規則性・先週比を評価する。
...
「今週」は **ISO週**（`date '+%G-W%V'`）で判定する。曜日で判定しない — 週次レビューは
土曜に回すことも日曜に回すこともあり、ISO週なら同じ週に入るので実施曜日が変わっても
二重に促さない。測定間隔が6〜8日で揺れるが、想起期間が2週間なので影響しない。
...
- 例: FFMI（monthly）の目標値に対して今週末の値はいくらで、目標までの残差・直近の伸びはどうか
