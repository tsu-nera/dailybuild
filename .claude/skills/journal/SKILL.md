...
- 数値は skeleton が持っている。**再掲せず、判断を左右した値だけ根拠として引く**
...
### Step 4: Weekly Summary を再生成

その週の日次エントリから `weekly:` 区間を作り直す（形式は weekly 節）。**週の途中でも毎回やる。** 週末に `--weekly` を叩き忘れると要約は永久に空欄のままで、実際 W33-W36 が4週ぶんそうなった。区間外の `Next Week's Adjustments` は残す。
