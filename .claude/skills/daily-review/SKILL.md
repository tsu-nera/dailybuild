...
Fitbit / HealthPlanet / 日次記録の失敗だけで、その場合はどのデータが欠けた
状態でレビューしているかを明示する。
...
- STATE.md のパイプライン表で `要確認` のものは Step 3 で扱う。ただし健康以外
  （MF・Toggl）は本文に持ち込まず、末尾で1行触れるだけにする
...
- カロリー収支とタンパク質摂取量（体重×1.6-1.8g/日。1.6が下限、カロリー赤字の日は1.8寄り。2.0は上限で必達ラインではない）
