...
- 体重トレンド（kg/週。カロリー収支には換算しない）とタンパク質摂取量（g/kg/日、1.6-1.8が下限、2.0は上限で必達ラインではない）
- 平日/週末の食事パターンの違い
...
- `config/exercise_muscles.yaml` の `weekly_sets` に対する達成/未達を記録する。**目標値は書き換えない**（2026-W39〜W41 は固定して実測を取る期間。昇級条件は実測が出てから別 issue で決める）
