...
（選択式グリッド「いまの状態」1〜5（いまの気分の1行）・気持ち12個・
何があった？）。**Issue #104** で、気分1問だけの scaleQuestion から
グリッド1問（`questionGroupItem`）へ移行した（身体・頭の疲労感を manual.csv の
1日1点でなく timestamp 付きで取るため）。

- **2026-09-07 に 身体の軽さ / 頭の軽さ の2行を外した。** 同じ軸を日次記録の
  朝・夜フォーム（`daily.py`）が取っており、二重に記録していたため。行は
  **末尾から**削除したので `いまの気分` の questionId は保持され、過去回答は
  そのまま読める（先頭・中間の削除は行順変更と同じ事故になる。
  `test_sync_questions_grid_remove_trailing_rows_preserves_first_row_id`）。
  グリッド自体は残るので kind は変わらず、`--allow-kind-replace` は不要。
  `data/emotion.csv` の `body` / `head` 列は 2026-08-31〜09-07 の値を持った
  まま残る（マージが `preserve_existing_on_nan=True` のため消えない）。
  読む側（`src/lib/emotion/`）は両列を見ない
...
- グリッドの行ごとの必須/任意は `config/emotion_def.yaml` の `grid_required`
  ...
  list を受け取る）。行を足すときは既定を任意にすること。全行必須だと気分だけ
  記録したいときも毎回すべて答える必要が生じ、記録が続くコストに直接効く
- グリッドの列（1〜5）は **valence（高=良好）であって感情・身体感覚の強さでは
  ...
  （`emotion.py`）で行う
- **yaml を先に改名すると `fetch` が実行できなくなる。** 移行手順に「先に
  `fetch` して未取得回答を materialize する」とあるが、それは
  `--allow-kind-replace` で質問が `deleteItem` される破壊的経路のためのガード
  （`has_unfetched_responses()`）。タイトル変更のみ（questionId 保持）の場合は
  `build_dataframe()` が `ValueError: フォームに質問がない` で落ちるので、
  先に `setup-form --update` を回してよい（2026-09-05 の改名で実際に踏んだ）
- **行タイトルの改名は questionId を保つが、意味は変える。** 2026-09-05 に
  `頭の冴え` → `頭の軽さ` へ改名した（身体の軽さと同じ軸に揃えるため。グリッドの
  列見出しは全行共通で向きが「高=良好」に強制されるので、「重さ」では逆になる）。
  `build_dataframe()` は現在のフォームから `title -> questionId` を引くため、
  **改名後も過去回答は同じ `head` 列に着地し続ける**。列は連続するが構成概念が
  変わっているので、改名前の24件（2026-08-31〜09-05）と接いで読まないこと。
  版は `data/emotion_grid_history.csv` にある（行そのものは 09-07 に廃止）
...
- **`data/emotion_grid_history.csv` にグリッドの行構成（行タイトルの並び順）の
  ...
  「何の版か」が読み取れなくなるため。身体・頭の列は 2026-08-31〜09-07 に
  しか値が入らず、これが無いと空欄が「軽かった」のか「まだ聞いていなかった／
  もう聞いていない」のか区別できない
...
- **満足感・達成感（夜）を BATD-R の楽しさ／重要さと混ぜない。**
  前者は1日1点の粗い常時チャネル（Beck の Mastery/Pleasure）、後者は
  1時間枠ごとの手書きスパースな高解像度チャネルで母集団が違う。後者の
  実装（`data/activity.csv`）は 2026-09-07 に廃止したが、行動活性化を
  再開したときに同じ混同をしないよう残す（docs/batdr.md）
...
- **Drive フォルダへの移動は `setup-form` の新規作成時のみ。** `forms.create`
  ...
  `drive.file` のみ）を持つ。
  **Drive API は OAuth クライアントの GCP プロジェクトで別途有効化が要る**
  （Forms API が有効でも足りず、無効なら 403 `accessNotConfigured`）。
  夜フォームの実作成でこれを踏み、2026-09-06 に有効化した。以後は自動で
  移動する。移動に失敗してもフォーム自体は作成済みで `form_id` は保存
  されるので、警告を出して続行する
